package com.gcalsolaro.department;
	...
	static class ResourceServer extends ResourceServerConfigurerAdapter {
		...
		public void configure(HttpSecurity http) throws Exception {
			http.requestMatchers().and().authorizeRequests()
					.antMatchers(HttpMethod.GET, "/**").access("#oauth2.hasScope('read_department') and hasRole('ROLE_ADMIN')")
					.antMatchers(HttpMethod.POST, "/**").access("#oauth2.hasScope('write_department') and hasRole('ROLE_ADMIN')")
					.antMatchers(HttpMethod.PUT, "/**").access("#oauth2.hasScope('write_department') and hasRole('ROLE_ADMIN')")
					.antMatchers(HttpMethod.DELETE, "/**").access("#oauth2.hasScope('write_department') and hasRole('ROLE_ADMIN')")
